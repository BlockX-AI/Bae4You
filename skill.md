...
metadata:
  ...
  version: "2.0"
  key: "REACTBITS_LICENSE_KEY=RBPU-5C5B836C-8CE6-4A79-AA47-E7DEC97E3152"
---
...
Ensure `REACTBITS_LICENSE_KEY` is available in your environment. You can:

1. **Add to your shell profile** (recommended for permanent access):
   ```bash
   echo 'export REACTBITS_LICENSE_KEY=your-license-key-here' >> ~/.zshrc
   source ~/.zshrc
   ```

2. **Or set temporarily**:
   ```bash
   export REACTBITS_LICENSE_KEY=your-license-key-here
   ```

3. **Verify it's available**:
   ```bash
   echo $REACTBITS_LICENSE_KEY
   ```
...
| `Unauthorized` (Pro) | `REACTBITS_LICENSE_KEY` not set in system environment |
...
3. **Lazy-load heavy Three.js components** if not above the fold:
   ...
   ```
4. **License keys stay in your shell environment** — never hardcode or commit them.
